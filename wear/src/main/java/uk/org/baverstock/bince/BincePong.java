...
//                dumpView(rootView, "", 30);
    ...
    private void dumpView(View view, String pfx, int i) {
        if (i < 0) {
            Log.d("Bince", pfx + "Cut");
            return;
        }
        if (view == null) {
            Log.d("Bince", pfx + "NO VIEW");
            return;
        }
        Log.d("Bince", pfx + view);
        if (view instanceof ViewGroup) {
            int childCount = ((ViewGroup) view).getChildCount();
            Log.d("Bince", pfx + "children = " + childCount);
            for (int child = 0; child < childCount; child++) {
                dumpView(((ViewGroup) view).getChildAt(child), ". " + pfx, i - 1);
            }
        }
    }
